(_ => {
	...
	window.onload = function () {
		...
		const createAddonList = (type, ending) => {
			let [header, list] = createLoadingList(`https://raw.githubusercontent.com/ItsJustJoshDev/Library/main/${type}/README.md`, {
				...
				getChildren: response => response.split("- [").splice(1).map(n => {
					...
					return name && (!createAddonList.search[type] || [name, description].filter(n => n).some(s => s.toLowerCase().indexOf(createAddonList.search[type].toLowerCase()) > -1)) && createAddonCard({
						...
						resourceUrl: `https://github.com/ItsJustJoshDev/Library/blob/main/${type}/${fileName}/_res`,
						websiteUrl: `https://github.com/ItsJustJoshDev/Library/blob/main/tree/main/${type}/${fileName}`,
						sourceUrl: `https://github.com/ItsJustJoshDev/Library/blob/main/${type}/${fileName}/${fileName}${ending}`,
		...
		const createLibraryEntry = _ => {
			return createLoadingList("https://github.com/ItsJustJoshDev/Library/blob/main/Library/README.md", {
				className: "addon-list",
		...
		const sections = [
			...
			{section: "General", renderSection: _ => [
				...
				createElement(`<div class="welcome-details">On here you'll find all my Plugins and Themes that are hosted on my <a href="https://github.com/ItsJustJoshDev/Library" target="_blank">GitHub</a>.</div>`),
				createElement(`<div class="welcome-details">If you expierence any Issues or got some Improvement Suggestions for my stuff, then feel free to drop them on <a href="https://github.com/ItsJustJoshDev/Library/issues" target="_blank">here</a>. But please make sure whether there is already an open Thread for your Issue or Suggestion.</div>`),
			]},
			{section: "Separator"},
		...
		const renderContent = data => {
			...
			document.title = `Library - ${data.section}`;
		};
